...
server <- function(input, output, session) {
  ...
  output$combined <- renderPlotly({
    ...
    # Create ggplot object
    cm <- cm %>%
      arrange(CMDECOD, ASTDY) %>%
      mutate(
        CMDECOD = factor(CMDECOD, levels = unique(CMDECOD)),
    ...
    p1 <-
      ggplot(
        ...
        aes(
          xmin = ASTDY,
          xmax = AENDY,
          ymin = as.numeric(CMDECOD) - 0.3,
          ymax = as.numeric(CMDECOD) + 0.3,
          fill = CMCLAS,
          text = TOOLTIP
        )
      ) +
      geom_rect() +
      # scale_x_continuous(limits = c(xmin(), xmax()))+
      scale_y_continuous(breaks = 1:length(levels(cm$CMDECOD)),
                         labels = levels(cm$CMDECOD)) +
      labs(y = "Concomitant Medication", x = "Study Day") 
    
    # Convert to plotly object
    o2 <- ggplotly(p1, tooltip = 'text')  
    
    combined_plot <- subplot(o1, o2, nrows = 2, shareX = TRUE)
  ...
  output$cm_meta <- renderTable({
    ...
    # Select metadata columns (adjust this to select the columns you want to display)
    cmmeta <-
      cm %>% select(USUBJID, TRTA, TRTSDT, TRTEDT, ASTDY, AENDY, CMDECOD, CMCLAS)
